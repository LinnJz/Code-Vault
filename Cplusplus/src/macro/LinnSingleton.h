...
#define UNIQUE(CLASS) LINN_PAIR(UNIQUE, CLASS)
#define SHARED(CLASS) LINN_PAIR(SHARED, CLASS)
#define MEYERS(CLASS) LINN_PAIR(MEYERS, CLASS)

#define NO_DESTROY          NO_DESTROY
#define ENABLE_DESTROY      ENABLE_DESTROY
#define ENABLE_LOCK_DESTROY ENABLE_LOCK_DESTROY

#define SINGLETON_CREATE(...)                       LINN_CAT(SINGLETON_CREATE_, LINN_NARGS(__VA_ARGS__))(__VA_ARGS__)
#define SINGLETON_CREATE_1(MODE_PAIR)               SINGLETON_CREATE_2(MODE_PAIR, NO_DESTROY)
...
public:                                                                                             \
  ...
  {                                                                                                 \
    ...
    std::call_once(flag, [&]() { s_Instance = _InstanceCreateImpl(std::forward<Args>(args)...); }); \
...
#define LINN_SINGLETON_CREATE_FROM_PAIR(MODE_PAIR, DESTROY_MODE)                                                   \
  LINN_CAT4(LINN_SINGLETON_CREATE_BODY_, LINN_PAIR_FIRST(MODE_PAIR), _, DESTROY_MODE)(LINN_PAIR_SECOND(MODE_PAIR))
