 ...
 *     via radix_traits::sort_mode (default = LSD)
 *   - Loop unrolling, cache‑friendly bucket processing, and OpenMP parallelism
 *
 * The user can choose the sorting strategy via radix_traits::sort_mode (lsd or msd).
 ...
 *      .sort_order = stdex::radix_traits::desc,
 *      .nan_pos    = stdex::radix_traits::nan_at_end,
 *   };
 ...
 *      .sort_mode = stdex::radix_traits::msd
 *   };
 ...
 * namespace stdex {
 *
 *   enum class radix_traits { asc, desc };
 *   enum class radix_traits { unhandled, begin, end };
 *
 *   struct radix_traits {
 *     enum mode { lsd, msd };
 ...
 *     radix_traits   sort_order = asc;
 *     radix_traits nan_pos    = unhandled;
 *   };
 *
...
struct radix_traits
{
  ...
  enum nan_position
  {
    ...
    nan_unhandled, //< maybe output: -nan(ind) -inf -1 -0 0 1 3.14 inf nan
    nan_at_begin,  //< maybe output: -nan(ind) nan -inf -1 -0 0 1 3.14 inf
    nan_at_end     //< maybe output: -inf -1 -0 0 1 3.14 inf nan -nan(ind)
  } nan_pos { nan_unhandled };
...
Adjust_radix_traits_(radix_traits Traits_, bool Is_reverse_) noexcept
{
  if (Is_reverse_)
  {
    Traits_.sort_order = Traits_.sort_order == radix_traits::asc ? radix_traits::desc : radix_traits::asc;
  }
  return Traits_;
}
...
Radix_key_(Unsigned_ty_ Val_, size_t Radix_) noexcept
{
  ...
  constexpr uint16_t Mask_  = Radix_traits_.bucket_size - 1;
  constexpr uint16_t Shift_ = Radix_traits_.bucket_size == 256U ? 8 : 16;

  if constexpr (Radix_traits_.sort_order == radix_traits::asc)
  {
    return (Val_ >> Shift_ * Radix_) & Mask_;
  }
  else /* Radix_traits_.sort_order == radix_traits::desc */
  {
    return Mask_ - ((Val_ >> Shift_ * Radix_) & Mask_);
  }
}

template<Radix_projection_tag_ Tag_, radix_traits Radix_traits_, typename Ty_, typename Proj_functor_>
ALWAYS_INLINE size_t
Radix_key_(Ty_ const &Val_, size_t Radix_, Proj_functor_ const &Proj_func_) noexcept
{
  return Radix_key_<Radix_traits_>(Proj_func_(Tag_ {}, Proj_func_.Get_key_value_(Val_)), Radix_);
}

template<radix_traits Radix_traits_, typename Ty_, typename Proj_functor_>
ALWAYS_INLINE size_t
Radix_key_(Ty_ const &Val_, size_t Radix_, Proj_functor_ const &Proj_func_) noexcept
{
  return Radix_key_<Radix_projection_functor_default_tag_, Radix_traits_, Ty_, Proj_functor_>(Val_, Radix_, Proj_func_);
}
...
template<radix_traits Radix_traits_>
struct Radix_chunk_context_
{
  using Chunk_ptr_ty_ = size_t (*)[Radix_traits_.bucket_size];
  ...
  {
    Buffer_size_ = static_cast<size_t>(Radix_traits_.bucket_size) * static_cast<size_t>(Threads_num_);
...
template<bool Is_last_pass_, radix_traits Radix_traits_, typename Ty_, typename Proj_functor_>
ALWAYS_INLINE void
Radix_lsd_histogram_(Ty_ const &Val_,
                     size_t (*const Chunks_)[Radix_traits_.bucket_size],
                     size_t                             Thread_id_,
                     size_t                             Radix_,
                     ...
                     [[maybe_unused]] std::span<size_t> NaN_counts_ = {})
{
  ...
  {
    if constexpr (Is_last_pass_ and Radix_traits_.nan_pos != radix_traits::nan_unhandled)
    {
    ...
    {
      ++Chunks_[Thread_id_][Radix_key_<Radix_traits_>(Val_, Radix_, Proj_func_)];
    }
    else
    {
      ++Chunks_[Thread_id_][Radix_key_<Radix_traits_>(
  ...
  if constexpr (std::integral<typename Proj_functor_::Key_ty_>)
  {
    ...
    {
      ++Chunks_[Thread_id_]
               [Radix_key_<Radix_traits_>(Proj_func_(Radix_int_xor_tag_, Proj_func_.Get_key_value_(Val_)), Radix_)];
    }
    else
    {
      ++Chunks_[Thread_id_][Radix_key_<Radix_traits_>(
                   ...
                   [[maybe_unused]] size_t &NaN_offset_ = 0)
{
  if constexpr (std::floating_point<typename Proj_functor_::Key_ty_>)
  {
    ...
    if constexpr (Is_last_pass_)
    {
      Output_[--Chunks_[Thread_id_][Radix_key_<Radix_traits_>(Begin_[Index_], Radix_, Proj_func_)]] =
          std::move(Begin_[Index_]);
    }
    else
    {
      Output_[--Chunks_[Thread_id_][Radix_key_<Radix_traits_>(
...
#pragma omp parallel if (Paral_data_.Threads_num_ > 1)
    {
      ...
      {
        Radix_lsd_histogram_<Is_lass_pass_, Radix_traits_>(Begin_[Index_], Chunks_, Thread_id_, Radix_, Proj_func_,
                                                           NaN_counts_);
      });
...
#pragma omp single
      {
        if constexpr (std::floating_point<typename Proj_functor_::Key_ty_> and
                      Radix_traits_.nan_pos != radix_traits::nan_unhandled and Is_lass_pass_)
        {
          ...
          if constexpr (Radix_traits_.nan_pos == radix_traits::nan_at_begin)
          {
            for (size_t Thread_id_ = 0; Thread_id_ < static_cast<size_t>(Paral_data_.Threads_num_); ++Thread_id_)
            {
          ...
          else if constexpr (Radix_traits_.nan_pos == radix_traits::nan_at_end)
          {
...
#pragma omp barrier
      ...
      if (Need_collect_)
      {
        ...
        Unroll_loop_<Radix_traits_.unroll_n>(Beg_index_, End_index_, [&](size_t Index_)
        {
          Radix_lsd_collect_<Is_lass_pass_, Radix_traits_>(Begin_, Size_, Beg_index_ + (End_index_ - 1 - Index_),
                                                           Output_, Chunks_, Thread_id_, Radix_, Proj_func_,
                                                           Thread_nan_offset_);
        });
      }
    }
  ...
  else
  {
    ...
    {
      Radix_lsd_histogram_<Is_lass_pass_, Radix_traits_>(Begin_[Index_], Chunks_, 0, Radix_, Proj_func_, NaN_counts_);
    });
    ...
    Unroll_loop_<Radix_traits_.unroll_n>(0, Size_, [&](size_t Index_)
    {
      Radix_lsd_collect_<Is_lass_pass_, Radix_traits_>(Begin_, Size_, Size_ - 1 - Index_, Output_, Chunks_, 0, Radix_,
                        ...
                        Proj_functor_ const                   &Proj_func_)
{
  ...
  Radix_chunk_context_<Radix_traits_> Chunk_ctx_;
  ...
  //< But this will waste stack memory of (Radix_traits_.bucket_size)
  //< So we use (alloca)

  else // if constexpr (!Is_parallel_)
  {
    size_t Stack_bucket_buffer_[Radix_traits_.bucket_size];
  ...
  */
  ...
  for (size_t Curr_radix_ = 0; Curr_radix_ < Radix_ - 1; ++Curr_radix_)
  {
    ...
    LSD_integer_radix_pass_<Is_parallel_, false, Radix_traits_>(Paral_data_, Begin_, Size_, Output_, Chunks_,
                                                                Curr_radix_, Proj_func_);
  }

  {
    ...
    if constexpr (std::floating_point<typename Proj_functor_::Key_ty_> and
                  Radix_traits_.nan_pos != radix_traits::nan_unhandled)
    {
    ...
    LSD_integer_radix_pass_<Is_parallel_, true, Radix_traits_>(Paral_data_, Begin_, Size_, Output_, Chunks_, Radix_ - 1,
...
template<radix_traits Radix_traits_, typename Random_point_, typename Random_buffer_point_, typename Proj_functor_>
inline void
                                       ...
                                       [[maybe_unused]] size_t                Deep_ = 0)
{
  ...
  if constexpr (Radix_traits_.bucket_size == 256U)
  {
    Radix_ = Radix_ + 1;
  }
  else /*if constexpr (Radix_traits_.bucket_size == 65536U)*/
  {
  ...
  {
    size_t Chunks_[Radix_traits_.bucket_size] {};

    // Histogram
    Unroll_loop_<Radix_traits_.unroll_n>(0, Size_, [&](size_t Index_)
    {
      ++Chunks_[Radix_key_<Radix_traits_>(Begin_[Index_], Curr_radix_, Proj_func_)];
    });
    ...
    for (; Radix_traits_.unroll_n <= Size_ - Index_; Index_ += Radix_traits_.unroll_n)
    {
      for (size_t Roll_ = 0; Roll_ < Radix_traits_.unroll_n; ++Roll_)
      {
        Output_[--Chunks_[Radix_key_<Radix_traits_>(Begin_[Size_ - 1 - (Index_ + Roll_)], Curr_radix_, Proj_func_)]] =
    ...
    {
      Output_[--Chunks_[Radix_key_<Radix_traits_>(Begin_[Size_ - 1 - Index_], Curr_radix_, Proj_func_)]] =
                        ...
                        size_t                                 Deep_ = 0)
{
  ...
  {
    return MSD_recursion_exit_integer_radix_sort_<Radix_traits_>(Begin_, Size_, Output_, Radix_, Proj_func_, Deep_);
  }
  ...
  if constexpr (Is_parallel_)
  {
    ...
    if (omp_in_parallel())
    {
      return MSD_integer_radix_sort_<false, Radix_traits_>(Begin_, Size_, Output_, Radix_, Proj_func_, Deep_);
    }
  }

  // bool const Can_enable_parallel_ = Threads_num_ > 1 && !omp_in_parallel() && Size_ > Radix_traits_.chunk_size * 2;

  if constexpr (!Is_parallel_)
  {
    size_t Chunk_[Radix_traits_.bucket_size] {};

    Unroll_loop_<Radix_traits_.unroll_n>(0, Size_, [&](size_t Index_)
    {
      ++Chunk_[Radix_key_<Radix_traits_>(Begin_[Index_], Radix_, Proj_func_)];
    });
    ...
    if (Non_empty_count_ > 1)
    {
      Unroll_loop_<Radix_traits_.unroll_n>(0, Size_, [&](size_t Index_)
      {
        ...
        Output_[--Chunk_[Radix_key_<Radix_traits_>(Begin_[Curr_index_], Radix_, Proj_func_)]] =
      ...
      Unroll_loop_<Radix_traits_.unroll_n>(0, Radix_traits_.bucket_size - 1, [&](size_t Index_)
      {
        ...
        MSD_integer_radix_sort_<Is_parallel_, Radix_traits_>(New_output_, Chunk_[Index_ + 1] - Chunk_[Index_],
                                                             New_beign_, Radix_ - 1, Proj_func_, Deep_ + 1);
      });
      auto *New_beign_  = Begin_ + Chunk_[Radix_traits_.bucket_size - 1];
      auto *New_output_ = Output_ + Chunk_[Radix_traits_.bucket_size - 1];
      MSD_integer_radix_sort_<Is_parallel_, Radix_traits_>(New_output_, Size_ - Chunk_[Radix_traits_.bucket_size - 1],
                                                           New_beign_, Radix_ - 1, Proj_func_, Deep_ + 1);
    }
    else
    {
      ...
      MSD_integer_radix_sort_<Is_parallel_, Radix_traits_>(Begin_, Size_, Output_, Radix_ - 1, Proj_func_, Deep_);
    }
    return;
  }
  else
...
#pragma omp parallel if (Threads_num_ > 1)
    {
      ...
      Unroll_loop_<Radix_traits_.unroll_n>(Beg_index_, End_index_, [&](size_t Index_)
      {
        ++Chunks_[Thread_id_][Radix_key_<Radix_traits_>(Begin_[Index_], Radix_, Proj_func_)];
      });
    }
...
#pragma omp parallel if (Threads_num_ > 1)
    {
      ...
      Unroll_loop_<Radix_traits_.unroll_n>(Beg_index_, End_index_, [&](size_t Index_)
      {
        ...
        Output_[--Chunks_[Thread_id_][Radix_key_<Radix_traits_>(Begin_[Curr_index_], Radix_, Proj_func_)]] =
            std::move(Begin_[Curr_index_]);
      });
    }

#pragma omp parallel for schedule(guided, 1) if (Threads_num_ > 1)
    for (int Bucket_idx_ = 0; Bucket_idx_ < static_cast<int>(Radix_traits_.bucket_size); ++Bucket_idx_)
    {
      size_t Chunk_begin_ = Chunks_[0][Bucket_idx_];
      size_t Chunk_end_ =
          (Bucket_idx_ + 1 < static_cast<int>(Radix_traits_.bucket_size)) ? Chunks_[0][Bucket_idx_ + 1] : Size_;
      ...
      MSD_integer_radix_sort_<Is_parallel_, Radix_traits_>(New_output_, Chunk_size_, New_beign_, Radix_ - 1, Proj_func_,
                                                           Deep_ + 1);
    }
  }
}
                 ...
                 Proj_functor_ const                   &Proj_func_)
{
  ...
  if constexpr (Radix_traits_.sort_mode == radix_traits::msd)
  {
    MSD_integer_radix_sort_<Is_parallel_policy_v_<Expo_ty_>, Radix_traits_>(Begin_, Size_, Output_, Radix_, Proj_func_);
  }
  else
  {
    if constexpr (Radix_traits_.bucket_size == 256U)
    ...
    LSD_integer_radix_sort_<Is_parallel_policy_v_<Expo_ty_>, Radix_traits_>(Begin_, Size_, Output_, Radix_, Proj_func_);
  }
}
                       ...
                       Key_function_   &&Key_func_ = {})
{
  ...
  static_assert(!(Radix_traits_.bucket_size == 65536U && sizeof(typename Proj_functor_::Key_ty_) == 1),
  ...
  constexpr auto Adjust_traits_ = Adjust_radix_traits_(Radix_traits_, Is_reverse_iterator_v_<Random_iter_>);
  ...
  if constexpr (sizeof(typename Proj_functor_::Key_ty_) > 4)
  {
    if constexpr (std::integral<typename Proj_functor_::Key_ty_> or
                  (std::floating_point<typename Proj_functor_::Key_ty_> and
                   Adjust_traits_.nan_pos == radix_traits::nan_unhandled))
    {
      Max_val_ = Max_element_<false, Adjust_traits_.unroll_n>(Primary_addr_, Size_, Proj_func_);
    }
    else if constexpr (Adjust_traits_.nan_pos == radix_traits::nan_at_begin)
    {
    ...
    else /*if constexpr (Adjust_traits_.nan_pos == radix_traits::nan_at_end)*/
    {
      auto Max_with_part_ = Max_while_partition_(Primary_addr_, Primary_addr_ + Size_, [&](auto const &Val_)
      {
        return !std::isnan(Proj_func_.Get_key_value_(Val_));
  ...
  Radix_sort_impl_<Adjust_traits_>(std::forward<Exec_policy_>(Expo_), Primary_addr_, Sort_size_, Buffer_, Max_val_,
                                   Proj_func_);
}
...
{
  radix_sort<Radix_traits_>(std::execution::seq, Begin_, End_, std::forward<Key_function_>(Key_func_));
}
...
{
  radix_sort<Radix_traits_>(std::forward<Exec_policy_>(Expo_), std::nan_at_begin(C_), std::nan_at_end(C_),
                            std::forward<Key_function_>(Key_func_));
}
...
{
  radix_sort<Radix_traits_>(std::execution::seq, C_, std::forward<Key_function_>(Key_func_));
}
