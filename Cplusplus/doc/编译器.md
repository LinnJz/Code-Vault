...
以下列表总结了在 GCC 和 Clang 中常见的可配合 `-Werror` 使用的警告类型，它们在提升代码健壮性方面至关重要。

| 警告名称                        | 主要作用                                                     | 推荐级别 |
| ------------------------------- | ------------------------------------------------------------ | -------- |
| **`-Wreturn-type`**             | 检测函数的返回路径是否缺少`return`语句，防止未定义行为。     | 强烈推荐 |
| **`-Wunused-variable`**         | 提醒清理未使用的变量，保持代码整洁。                         | 推荐     |
| **`-Wunused-but-set-variable`** | 检测变量被赋值后从未使用，可能是指示逻辑错误。               | 推荐     |
| **`-Wunused-parameter`**        | 提醒处理函数中未使用的参数，有助于接口设计清晰。             | 推荐     |
| **`-Wformat`**                  | 检查`printf`、`scanf`等函数的格式化字符串与参数类型是否匹配。 | 强烈推荐 |
| **`-Wconversion`**              | 检测可能改变值的隐式类型转换，例如从`double`到`int`的转换。  | 可选     |
| **`-Wsign-compare`**            | 警告有符号数与无符号数之间的比较，这种比较可能产生非预期结果。 | 推荐     |
| **`-Wswitch`**                  | 确保`switch`语句处理了`enum`的所有可能值，提高代码的健壮性。 | 推荐     |
| **`-Waddress`**                 | 检查可疑的内存地址使用，例如将某变量的地址作为条件判断。     | 可选     |
| **`-Wempty-body`**              | 警告空的`if`、`else`或循环体，这通常是代码遗漏。             | 推荐     |
| **`-Warray-bounds`**            | 检测数组越界访问，防止内存破坏。                             | 强烈推荐 |
| **`-Wcast-function-type`**      | 警告不兼容的函数指针类型转换，可能导致运行时崩溃。           | 强烈推荐 |
| **`-Wshadow`**                  | 警告变量名遮蔽（shadow）现象，防止作用域混淆引发的逻辑错误。 | 推荐     |
| **`-Wimplicit-fallthrough`**    | 检测`switch`语句中是否遗漏`break`或明确的`[[fallthrough]]`标记。 | 推荐     |
| **`-Wformat-security`**         | 检测格式化字符串是否被外部数据控制，防止格式化字符串漏洞。   | 强烈推荐 |
| **`-Wdouble-promotion`**        | 警告在运算中将`float`隐式提升为`double`，这可能无意中使用了双精度浮点数。 | 可选     |

## 包含关系对照表

| 警告名称                    | 包含于 `-Wall` | 包含于 `-Wextra` | 备注                                  |
| --------------------------- | -------------- | ---------------- | ------------------------------------- |
| `-Wreturn-type`             | ✅              | ❌                | 基础警告，`-Wall` 启用                |
| `-Wunused-variable`         | ✅              | ❌                | `-Wall` 启用                          |
| `-Wunused-but-set-variable` | ✅              | ❌                | GCC/Clang 均在 `-Wall` 中             |
| `-Wunused-parameter`        | ❌              | ✅                | 仅在 `-Wextra` 中                     |
| `-Wformat`                  | ✅              | ❌                | `-Wall` 启用                          |
| `-Wconversion`              | ❌              | ❌                | **需要显式开启**，不在任何默认组      |
| `-Wsign-compare`            | ❌              | ✅                | 仅在 `-Wextra` 中                     |
| `-Wswitch`                  | ✅              | ❌                | `-Wall` 启用                          |
| `-Waddress`                 | ✅              | ❌                | `-Wall` 启用                          |
| `-Wempty-body`              | ❌              | ✅                | 仅在 `-Wextra` 中                     |
| `-Warray-bounds`            | ✅              | ❌                | `-Wall` 启用（部分边界检查）          |
| `-Wcast-function-type`      | ❌              | ✅                | GCC 8+ / Clang 均在 `-Wextra` 中      |
| `-Wshadow`                  | ❌              | ❌                | **需要显式开启**（`-Wshadow` 独立）   |
| `-Wimplicit-fallthrough`    | ❌              | ✅                | GCC 7+ / Clang 均在 `-Wextra` 中      |
| `-Wformat-security`         | ❌              | ❌                | **需要显式开启**（但依赖 `-Wformat`） |
| `-Wdouble-promotion`        | ❌              | ❌                | **需要显式开启**                      |
