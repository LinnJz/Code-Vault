...
- 代表 **multi-media extension**（多媒体扩展），这是SSE指令集家族（SSE, SSE2, SSE3等）的标准前缀。

- 也表示该函数返回一个 **128位** 的数据类型（通常是 `__m128` 或 `__m128d`、`__m128i`）。
...
- 第一个 `s`：**scalar**（标量） - 只操作一个数据元素
- 第二个 `s`：**single-precision floating-point**（单精度浮点数） - 32位浮点数
- 所以 `ss` 表示“标量单精度浮点数”

`ps`示例：

- **`ps` = packed single-precision floating-point**
  - **第一个字母 `p`**：**packed**（打包的/向量化的） - 同时操作多个数据元素
  - **第二个字母 `s`**：**single-precision floating-point**（单精度浮点数） - 32位浮点数
- 所以 `ps` 表示 **"打包的单精度浮点数"**，即同时对多个单精度浮点数进行向量化操作。
