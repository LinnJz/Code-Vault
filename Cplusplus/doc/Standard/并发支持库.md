...
### （4）示例4（__asm 指令 内存屏障 标准库std::atomic_thread_fence）
...
线程亲和性（Thread Affinity）是指将线程强制绑定到指定的一个或多个 CPU 逻辑核心上运行，从而禁止操作系统调度器将线程迁移到其他核心。通过合理设置亲和性，可以显著改善多线程程序在共享数据场景下的缓存效率与执行可预测性。

## 核心概念

- **操作系统默认行为**：为了均衡负载，操作系统会随时将线程在不同核心之间迁移。迁移后线程私有的 L1/L2 缓存失效，需从更远的共享缓存（L3）或内存重新加载数据，产生数十至上百时钟周期的额外开销。

- **缓存一致性协议**：当多个核心上的线程共享并修改同一数据时，硬件需要通过 MESI 等协议在各核心间同步缓存行。核心间的物理距离（是否共享 L3 缓存、是否位于同一 CPU 片内）决定了通信延迟。同一物理核心内的两个超线程共享 L1/L2 缓存，数据同步开销最低。

- **亲和性的作用**：
  - 消除因线程迁移导致的缓存完全失效问题。
  - 允许开发者根据硬件拓扑，将高频通信的线程绑定到共享缓存的同一物理核心（或其超线程）上，将数据同步延迟降至最低。
  - 提高 CPU 缓存命中率，减少核心间总线流量。

## 最佳实践

对于需要频繁共享并写入同一数据的两个线程，最优绑定策略为：**将它们绑定到同一个物理核心的两个逻辑处理器（超线程）上**。若分配到不同物理核心，即使使用亲和性固定位置，每次数据同步仍需经过 L3 缓存或核心间互联，延迟比共享 L1/L2 高一个数量级。

> 注：具体核心拓扑需通过系统工具（Linux `lstopo`、Windows `Coreinfo`）或 CPUID 指令获取。例如 Intel i5-10200H（4 核 8 线程）中，逻辑处理器 0 与 1 通常位于同一物理核心。

## 代码示例

以下示例使用 C++20 的 `std::jthread`，通过 `native_handle()` 获取底层线程句柄，并调用平台 API 设置亲和性掩码。使用预处理指令区分 Windows 与 Linux。
...
// 设置当前线程的 CPU 亲和性掩码
// 参数 mask: 位掩码，每一位代表一个逻辑处理器（从 0 开始）
void set_thread_affinity(std::jthread& t, unsigned long long mask) {
    auto native = t.native_handle();

#ifdef _WIN32
    // Windows: SetThreadAffinityMask 返回之前的掩码，0 表示失败
    DWORD_PTR result = SetThreadAffinityMask(native, static_cast<DWORD_PTR>(mask));
    assert(result != 0 && "SetThreadAffinityMask failed");
#elif defined(__linux__)
    // Linux: 使用 cpu_set_t，支持最多 1024 个 CPU
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    ...
    assert(ret == 0 && "pthread_setaffinity_np failed");
#else
#error "Unsupported platform"
#endif
}

// 示例：两个线程共享数据，绑定到同一物理核心的两个逻辑处理器（假设 CPU 0 和 CPU 1 共享 L1/L2）
int main() {
    int shared_data = 0;

    std::latch writer_latch(1);
    std::latch reader_latch(1);

    // 线程 A：写入数据
    std::jthread writer([&shared_data, &writer_latch] {
        writer_latch.wait();   // 等待启动信号
        for (int i = 0; i < 100000; ++i) {
            shared_data = i;
        }
    });
    ...
    // 线程 B：读取数据
    std::jthread reader([&shared_data, &reader_latch] {
        reader_latch.wait();   // 等待启动信号
        int local_sum = 0;
        for (int i = 0; i < 100000; ++i) {
            local_sum += shared_data;
        }
        std::cout << "Reader sum: " << local_sum << '\n';
    });

    // 主线程设置 CPU 亲和性
    set_thread_affinity(writer, 0b0001);   // 绑定到逻辑 CPU 0
    set_thread_affinity(reader, 0b0010);   // 绑定到逻辑 CPU 1

    // 发出启动信号
    writer_latch.count_down();
    reader_latch.count_down();
...
- `set_thread_affinity` 函数接受 `std::jthread&` 和一个掩码，通过 `native_handle()` 获取平台相关的线程句柄。
- 断言用于验证亲和性设置是否成功。生产代码中可替换为异常或日志。
- 掩码示例中 `0b0001`（CPU 0）和 `0b0010`（CPU 1）假设这两个逻辑处理器属于同一物理核心。实际使用时需根据 `GetLogicalProcessorInformation`（Windows）或 `/sys/devices/system/cpu/`（Linux）确认拓扑关系。
- 若需将两个线程绑定到同一物理核心的所有逻辑处理器，可将掩码同时包含两个位（如 `0b0011`），但这样每个线程仍可能在该核心的两个超线程间迁移，无额外收益。

## 注意事项

- 亲和性绑定会降低操作系统调度灵活性，在核心资源紧张时可能导致负载不均。仅对性能关键且通信密集的线程使用。
- 超线程共享执行单元，若两个线程均为计算密集型，同一核心内可能产生资源竞争，此时应权衡是否拆分为不同物理核心。
- 移动端或可变频率环境下，绑定核心可能影响功耗与温控策略。
- 设置亲和性需要相应权限（通常普通用户即可），但某些容器或虚拟化环境可能限制该操作。

# `<atomic>` 和 `<mutex>`并发同步解析基于MSVC

## 1. 原子类型何时使用硬件 CAS 指令，何时退化为锁？

- **1、2、4、8 字节**的原子类型：
  使用硬件提供的原子指令（如 `_InterlockedCompareExchange8`、`_InterlockedExchangeAdd64` 等），这些指令直接对应 CPU 的 `lock cmpxchg`、`lock xadd` 等，属于 **无锁（lock‑free）** 实现。
- **其他小于 16 字节的大小（3、5、6、7、9~15 字节）以及 16 字节的普通 `atomic<T>`**：
  没有对应的硬件原子指令，因此 `<atomic>` 会退化为 **基于锁** 的实现，即用互斥锁保护整个对象。
- **对于 `atomic_ref<T>`**：
  若 `sizeof(T)` 不是 1、2、4、8（或 16 且为引用特化），也会退化为使用互斥锁。

## 2. 退化为锁时，自旋锁和互斥锁分别用于什么类型？

源码中 `_Atomic_storage_types` 区分了两种锁：
...
struct _Atomic_storage_types {
    using _Spinlock = long;          // 用于 atomic<T>
};

template <class _Ty>
struct _Atomic_storage_types<_Ty&> {
    using _Spinlock = _Smtx_t*;      // 用于 atomic_ref<T>
};
```



- **`atomic<T>`（普通原子对象）**：使用 **自旋锁**（`long` 类型作为自旋变量）。
  自旋锁的实现（`_Atomic_lock_acquire` / `_Atomic_lock_release`）通过 `_InterlockedExchange` 原子交换（本质是 CAS 类指令）忙等待，不会让出 CPU。
- **`atomic_ref<T>`（原子引用）**：使用 **互斥锁**（`_Smtx_t*`，实际是 Windows SRW 锁）。
  互斥锁在等待时会使调用线程进入内核等待状态，适合长临界区。

## 3. 这些锁使用的硬件指令是 mutex 的硬件指令，还是 atomic 的 CAS 硬件指令？

- **自旋锁**：
  其实现直接使用 **atomic 的 CAS 类指令**（如 `_InterlockedExchange` `lock xchg`）来测试并设置锁变量。**不**涉及 `<mutex>` 中的 `std::mutex` 或 `condition_variable`。
- **互斥锁**（`_Smtx_t*`）：
  底层是 Windows SRW 锁（`SRWLOCK`）。它的内部实现混合了用户态原子操作（`lock cmpxchg` 等）和内核等待机制（当竞争激烈时进入内核）。虽然也用到 CAS 指令，但整体行为是操作系统提供的同步原语，不是简单的自旋。
- **`<mutex>` 中的 `std::mutex` 等**：
  这些标准互斥量在 MSVC 中底层也是用 `_Mtx_t`（即 `_Mtx_internal_imp_t*`），实际包装了 `SRWLOCK`（对于非递归）或临界区（对于递归）。**与 `atomic_ref` 退化所用的 `_Smtx_t\*` 本质上是同一套底层机制**（SRW 锁）。因此可以认为两者都依赖操作系统提供的锁原语，而非直接仅用 CAS 指令。

## 4. 总结对比表

| 原子对象类型                          | 实现方式         | 底层硬件/OS 原语                        | 是否使用 mutex 的锁机制？          |
| ------------------------------------- | ---------------- | --------------------------------------- | ---------------------------------- |
| `atomic<T>` 大小 1/2/4/8 字节         | 无锁（硬件原子） | `lock cmpxchg`、`lock xadd` 等          | 否                                 |
| `atomic<T>` 其他大小                  | 自旋锁           | `_InterlockedExchange`（原子交换，CAS） | 否（自旋锁不调用 mutex 函数）      |
| `atomic_ref<T>`（任意大小且非无锁）   | 互斥锁           | `_Smtx_t*`（Windows SRW 锁）            | 与 `std::mutex` 底层同源（SRW 锁） |
| `std::mutex` / `std::recursive_mutex` | 互斥锁           | `_Mtx_t`（也是 SRW 锁或临界区）         | 自身就是标准库锁                   |

所以结论：

- **自旋锁** 使用的是 **atomic 的 CAS 硬件指令**（`lock xchg`），不是 mutex 的锁。
- **互斥锁**（用于 `atomic_ref` 退化）与 `std::mutex` **使用相同的内核 SRW 锁机制**，因此可以认为它们共享一套底层同步原语，但 `atomic_ref` 的互斥锁并不通过 `<mutex>` 中的 `std::mutex` 类来实现。

## 5. 性能排序（一般情况）

| 实现方式                                    | 典型相对性能 | 关键特征                                                     |
| ------------------------------------------- | ------------ | ------------------------------------------------------------ |
| **无锁 (1/2/4/8 字节)**                     | **最高**     | 直接使用 CPU 原子指令，不涉及任何锁或内核                    |
| **自旋锁 (atomic<T> 退化)**                 | **次高**     | 忙等待，使用 `_InterlockedExchange` 等 CAS 指令自旋，但不会让出 CPU |
| **互斥锁 (atomic_ref<T> 退化, std::mutex)** | **最低**     | 使用操作系统同步原语（SRW 锁/临界区），竞争激烈时会进入内核等待 |

### 为什么自旋锁比互斥锁快？

- **自旋锁**：在用户态循环 `pause`/`yield`，等待时间很短时，避免了上下文切换和内核调用。
- **互斥锁**：即使是无竞争的 `mutex::lock()`，也可能需要用户态快速路径，但一旦有竞争，就会陷入内核（`WaitForSingleObject` 等），开销远大于自旋。

### 需要特别说明的边界情况

1. **长临界区**：**<u>如果被保护的代码段较长（例如几十微秒以上），自旋锁会浪费大量 CPU 时间，而互斥锁会阻塞线程、让出 CPU，整体系统吞吐量可能更高</u>**。但单线程等待性能仍然是互斥锁更“慢”（因为等待时间长是操作本身决定的，不是锁机制本身的延迟）。
2. **高竞争**：自旋锁在极高竞争下会 CPU 100% 空转，互斥锁则可以让线程睡眠。此时互斥锁可能“看起来”更好，但绝对延迟仍可能高于自旋锁（睡眠+唤醒开销）。通常需要结合场景衡量。
3. **atomic_ref 退化的互斥锁 vs std::mutex**：两者底层都是 Windows SRW 锁（或临界区），性能相当。`atomic_ref` 的互斥锁还额外有一个全局哈希表（`__std_atomic_get_mutex`），可能多一次间接开销，但通常可忽略。
